...
if sr != config["sampling_rate"]: #Resample to 24khz
    audio = librosa.resample(audio, sr, config["sampling_rate"])#, config["sampling_rate_for_feats"])
...
#                         hop_size=int(config["hop_size"] * sampling_rate/config["sampling_rate"]), 
#                         fft_size=int(config["fft_size"] * sampling_rate/config["sampling_rate"]),
...
wavedata = np.array(np.concatenate([wavedata, wavedata, wavedata, wavedata]))
